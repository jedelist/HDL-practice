...
architecture rtl of uart_rx is
    ...
    signal rx_meta : std_logic := '1';          -- Initialized to high since UART line IDLEs at high
    signal rx_sync : std_logic := '1';
begin

    rx_synchronizer : process (clk, rst)
    begin
        if (rst = '1') then
    ...
    begin
        if (rst = '1') then
            ...
            corrupt <= '0';              -- If a transmission is corrupt, only pulse for one cycle of this signal
            case state is 
                when IDLE => 
                    if (rx_sync = '0') then
